...
\definecolor{orange}{HTML}{D02670}
...
\node[service=orange, below=3cm of s3, xshift=1.8cm, minimum width=3.4cm]
  (processor) {\textbf{Lambda}\\[-1pt]\nsub{Document Processor}};

\draw[flow] (s3.south) |- node[elabel, above right, xshift=3pt, yshift=-4pt, text=black] {S3 event} (processor.west);

% === ROW 4: Processing Pipeline ===

\node[step=teal, below=1.1cm of processor, xshift=-4cm]
...
\begin{pgfonlayer}{background}
  ...
  % outer border removed for dark theme

  % Section containers removed
\end{pgfonlayer}
